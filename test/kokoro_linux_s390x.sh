...
# the test command to run under an emulated s390x docker container.
...
# use an actual s390x docker image (with a real s390x node) to run build & test grpc-js under an emulator
...
# TODO(jtattermusch): we're using s390x/node:12-stretch instead of s390x/node:12-buster because the buster-based image
...
docker run $DOCKER_TTY_ARGS --rm --user "$(id -u):$(id -g)" -e "HOME=/home/fake-user" -v "$(mktemp -d):/home/fake-user" -v "$(pwd)":/work -w /work s390x/node:12-stretch bash -c "$TEST_NODE_COMMAND"
